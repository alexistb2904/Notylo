...
export function PublicPage() {
  ...
  useEffect(() => {
    ...
    void loadPublicDocument(token)
      ...
      .catch((reason) => {
        if (active)
          setError(reason instanceof Error ? reason.message : "Ce lien public est indisponible.");
      });
    return () => {
      active = false;
    };
...
function LoadedPublicNotebook({
  ...
  const assetHashes = useRef<Readonly<Record<string, string>>>(
    Object.fromEntries(initial.assets.map((asset) => [asset.id, asset.hash]))
  );
  latestDocument.current = session.document;

  const savePublic = useCallback(
    async (document: NotebookDocument) => {
      if (saving.current) {
      ...
      try {
        ...
        const conflict =
          reason instanceof Error && "status" in reason && (reason as ApiError).status === 409;
...
async function loadPublicDocument(token: string): Promise<{
  document: NotebookDocument;
  revision: number;
  mode: ShareMode;
}> {
  ...
  if (!isNotebookDocument(result.document))
    throw new Error("La réponse du notebook partagé est invalide.");
  await Promise.all(
    result.document.assets.map(async (asset) => {
